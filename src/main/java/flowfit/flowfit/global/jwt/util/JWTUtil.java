package flowfit.flowfit.global.jwt.util;

import flowfit.domain.user.domain.entity.Role;
import blaybus.global.jwt.util.exception.BlaybusJWTException;
import blaybus.global.jwt.util.exception.BlaybusJWTExpiredException;
...
public class JWTUtil {
    ...
    public String getId(String token) {
        try {
            ...
            throw new BlaybusJWTExpiredException("JWT 가 만료되었습니다.");
        }
        catch(JwtException e) {
            throw new BlaybusJWTException("JWT token 로드에 실패했습니다.");
        }
    }
